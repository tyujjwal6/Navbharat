...
const Allotments = () => {
  ...
  const stopDrawing = (pageIndex) => {
    ...
    setIsDrawing(false);
    const canvas = canvasRefs[pageIndex].current;
    const signatureData = canvas.toDataURL('image/png'); // Use PNG for transparency

    const newSignatures = [...signatures];
    newSignatures.fill(signatureData); // Fill all with the same signature
    setSignatures(newSignatures);
    
    canvasRefs.forEach((ref, index) => {
      if (ref.current) {
        const ctx = ref.current.getContext('2d');
        ctx.clearRect(0, 0, ref.current.width, ref.current.height); // Clear first
        if (signatureData) {
          const img = new Image();
  ...
  const renderAgreementDetails = () => (
    <div className="space-y-4">
      <div className="text-center border-b pb-4">
        <h1 className="text-2xl font-bold text-gray-800">PROPERTY PURCHASE AGREEMENT</h1>
      ...
      <div className="grid md:grid-cols-2 gap-8">
        <Card>
          <CardHeader><CardTitle className="flex items-center gap-2"><User className="h-5 w-5" />Buyer Information</CardTitle></CardHeader>
          <CardContent className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4"><Label className="text-sm font-medium">Full Name</Label><p className="text-gray-800 border-b pb-1">{userData.name}</p></div>
            <div className="grid grid-cols-2 gap-4"><Label className="text-sm font-medium">S/o / D/o</Label><p className="text-gray-800 border-b pb-1">{userData.fatherName}</p></div>
            <div><Label className="text-sm font-medium">Address</Label><p className="text-gray-800 border-b pb-1">{userData.address}</p></div>
            <div className="grid grid-cols-2 gap-4"><Label className="text-sm font-medium">Phone</Label><p className="text-gray-800 border-b pb-1">{userData.phone}</p></div>
            <div className="grid grid-cols-2 gap-4"><Label className="text-sm font-medium">Email</Label><p className="text-gray-800 border-b pb-1">{userData.email}</p></div>
            <div className="grid grid-cols-2 gap-4"><Label className="text-sm font-medium">PAN</Label><p className="text-gray-800 border-b pb-1">{userData.panCard}</p></div>
            <div className="grid grid-cols-2 gap-4"><Label className="text-sm font-medium">Aadhaar</Label><p className="text-gray-800 border-b pb-1">{userData.aadhar}</p></div>
          </CardContent>
        </Card>
        <Card>
          <CardHeader><CardTitle className="flex items-center gap-2"><Home className="h-5 w-5" />Booking Details</CardTitle></CardHeader>
          <CardContent className="space-y-4">
            <div className="grid grid-cols-2 gap-4"><Label className="text-sm font-medium">Project</Label><p className="text-gray-800 border-b pb-1">{bookingData.project}</p></div>
            <div className="grid grid-cols-2 gap-4"><Label className="text-sm font-medium">Unit No.</Label><p className="text-gray-800 border-b pb-1">{bookingData.unitNumber}</p></div>
            <div className="grid grid-cols-2 gap-4"><Label className="text-sm font-medium">Area</Label><p className="text-gray-800 border-b pb-1">{bookingData.area}</p></div>
            <div className="grid grid-cols-2 gap-4"><Label className="text-sm font-medium">Total Cost</Label><p className="text-gray-800 font-semibold border-b pb-1">{bookingData.totalCost}</p></div>
            <div className="grid grid-cols-2 gap-4"><Label className="text-sm font-medium">Booking Amount</Label><p className="text-gray-800 font-semibold border-b pb-1">{bookingData.bookingAmount}</p></div>
            <div className="grid grid-cols-2 gap-4"><Label className="text-sm font-medium">Payment Plan</Label><p className="text-gray-800 border-b pb-1">{bookingData.paymentPlan}</p></div>
            <div className="grid grid-cols-2 gap-4"><Label className="text-sm font-medium">Mode</Label><p className="text-gray-800 border-b pb-1">{bookingData.modeOfPayment}</p></div>
          </CardContent>
        </Card>
      </div>
    </div>
  ...
  const renderSignatureSection = () => (
    <Card className="mt-8">
      <CardHeader><CardTitle className="flex items-center gap-2"><Pen className="h-5 w-5" />Digital Signature</CardTitle></CardHeader>
      <CardContent>
    ...
    <div className="w-full p-4 bg-gray-50 min-h-screen">
      ...
      <div style={{ position: 'absolute', left: '-9999px', top: 0 }}>
        <PrintableAgreement
          ...
          signatureDataUrl={signatures[0]} // Pass the saved signature data
        />
      </div>
      ...
      <div className="max-w-4xl mx-auto">
        <div className="flex w-full justify-between items-center mb-6">
          <Button variant="outline" size="sm" onClick={() => setCurrentPage(Math.max(0, currentPage - 1))} disabled={currentPage === 0}><ChevronLeft className="h-4 w-4 mr-2" />Previous</Button>
          <Badge variant="secondary" className="px-4 py-2 text-sm">{pages[currentPage]} (Page {currentPage + 1}/{pages.length})</Badge>
          <Button variant="outline" size="sm" onClick={() => setCurrentPage(Math.min(pages.length - 1, currentPage + 1))} disabled={currentPage === pages.length - 1}>Next<ChevronRight className="h-4 w-4 ml-2" /></Button>
        </div>
        ...
        {isSaved && (
          <div className="mt-8 text-center">
            <Button size="lg" className="px-8" onClick={handleDownloadPdf} disabled={isGenerating}>
              {isGenerating ? (
